...
<div class="bre-schema-metabox">
	<p>
		<label for="bre-schema-type"><strong><?php esc_html_e( 'Schema Type', 'bavarian-rank-engine' ); ?></strong></label><br>
		<select name="bre_schema[schema_type]" id="bre-schema-type">
			<option value="" <?php selected( $type, '' ); ?>><?php esc_html_e( '— No Schema —', 'bavarian-rank-engine' ); ?></option>
			<?php if ( in_array( 'howto', $enabled, true ) ) : ?>
			<option value="howto" <?php selected( $type, 'howto' ); ?>><?php esc_html_e( 'HowTo Guide', 'bavarian-rank-engine' ); ?></option>
			<?php endif; ?>
			<?php if ( in_array( 'review', $enabled, true ) ) : ?>
			<option value="review" <?php selected( $type, 'review' ); ?>><?php esc_html_e( 'Review / Rating', 'bavarian-rank-engine' ); ?></option>
			<?php endif; ?>
			<?php if ( in_array( 'recipe', $enabled, true ) ) : ?>
			<option value="recipe" <?php selected( $type, 'recipe' ); ?>><?php esc_html_e( 'Recipe', 'bavarian-rank-engine' ); ?></option>
			<?php endif; ?>
	...
	<div class="bre-schema-fields" data-bre-type="howto" style="display:none;">
		<p>
			<label><strong><?php esc_html_e( 'Guide Name', 'bavarian-rank-engine' ); ?></strong><br>
			<input type="text" name="bre_schema[howto_name]"
				value="<?php echo esc_attr( $data['howto']['name'] ?? '' ); ?>"
				class="widefat"></label>
		</p>
		<p>
			<label><strong><?php esc_html_e( 'Steps (one line = one step)', 'bavarian-rank-engine' ); ?></strong><br>
	...
	<div class="bre-schema-fields" data-bre-type="review" style="display:none;">
		<p>
			<label><strong><?php esc_html_e( 'Reviewed Product / Service', 'bavarian-rank-engine' ); ?></strong><br>
			<input type="text" name="bre_schema[review_item]"
				value="<?php echo esc_attr( $data['review']['item'] ?? '' ); ?>"
				class="widefat"></label>
		</p>
		<p>
			<label><strong><?php esc_html_e( 'Rating (1–5)', 'bavarian-rank-engine' ); ?></strong><br>
	...
	<div class="bre-schema-fields" data-bre-type="event" style="display:none;">
		...
		<p>
			<label><?php esc_html_e( 'Start Date', 'bavarian-rank-engine' ); ?><br>
			<input type="date" name="bre_schema[event_start]"
				value="<?php echo esc_attr( $data['event']['start'] ?? '' ); ?>"></label>
		</p>
		<p>
			<label><?php esc_html_e( 'End Date (optional)', 'bavarian-rank-engine' ); ?><br>
			<input type="date" name="bre_schema[event_end]"
				value="<?php echo esc_attr( $data['event']['end'] ?? '' ); ?>"></label>
		</p>
		<p>
			<label><?php esc_html_e( 'Location or URL', 'bavarian-rank-engine' ); ?><br>
